...
object Handler {
  def apply[F[_], T](messageQueue: MessageQueue[F, T],
                     onFailure: io.circe.Error => F[Unit])(implicit decoder: Decoder[T], evaluate: F[Unit] => Unit) =
    new Handler {
      override def message(headers: util.Map[_, _], body: String): Unit =
        evaluate {
          parse(body)
            .flatMap(json => decoder(json.hcursor))
            .fold(onFailure, messageQueue.enqueue)
        }
    }
}
